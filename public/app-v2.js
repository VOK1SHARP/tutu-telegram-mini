/* APP.JS — объединённая, исправленная версия
   Избегаем inline onclick в template literals, используем делегацию.
   Сохраняем логику: cart, orders, popularity, checkout, cloud.
*/
...
function createToast(text, options = {}){
  ...
  t.style.background = options.background || 'rgba(0,0,0,0.8)';
  t.style.color = 'white';
  ...
  app.innerHTML = `
    <div class="header fade-in">
      <div class="header-content">
        ...
        <div class="user-avatar" data-action="profile" title="${escapeHtml(fullName)}${username ? ` (${escapeHtml(username)})` : ''}">
          ${hasPhoto ? `<img src="${escapeHtml(userData.photo_url)}" alt="${escapeHtml(fullName)}" class="avatar-img" />` : `<i class="fas fa-user"></i>`}
    ...
    <div class="cart-footer fade-in" style="animation-delay:0.4s">
      <div class="cart-content">
        ...
        <button class="checkout-button" id="checkout-btn" data-action="checkout" disabled>Оформить заказ</button>
      </div>
    </div>
  `;
...
function loadPopularProducts(){
  ...
  container.innerHTML = popular.map(t => `
    <div class="product-card" data-action="showProduct" data-id="${t.id}">
      ...
      <div class="product-info">
        ...
        <button class="product-button" data-action="addToCart" data-id="${t.id}">+ В корзину</button>
      </div>
    </div>
  `).join('');
}
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <div class="modal-body" style="padding:10px;">
        ${teaCatalog.map(t => `
          <div class="catalog-item" data-action="showProduct" data-id="${t.id}" style="padding:12px;border-radius:10px;display:flex;gap:12px;align-items:center;margin-bottom:10px;background:#fff;">
            ...
            <div style="text-align:right;">
              ...
              <button data-action="addToCart" data-id="${t.id}" style="padding:6px 10px;border-radius:10px;background:#4CAF50;color:white;border:none;cursor:pointer;">+ Добавить</button>
            </div>
          </div>
        `).join('')}
      </div>
...
function showProduct(productId){
  closeAllModals();
  const product = teaCatalog.find(p => p.id === Number(productId));
      ...
      <div class="modal-body">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding-top:10px;border-top:1px solid #eee;">
          ...
          <div style="display:flex;gap:8px;">
            <button data-action="addToCart" data-id="${product.id}" style="padding:10px 14px;border-radius:10px;background:linear-gradient(135deg,#4CAF50,#2E7D32);color:white;border:none;cursor:pointer;">Добавить</button>
            <button data-action="catalog" style="padding:10px 14px;border-radius:10px;background:#eee;border:none;cursor:pointer;">Каталог</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="modal-body">
          ...
          ${cart.length === 0 ? `<div style="text-align:center;padding:40px 10px;color:#888;"><i class="fas fa-box-open" style="font-size:42px;color:#ddd;"></i><div>Корзина пуста</div></div>` : cart.map(item => `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-radius:10px;background:#f8f9fa;margin-bottom:10px;">
              ...
              <div style="display:flex;align-items:center;gap:10px;">
                <button data-action="changeQty" data-id="${item.id}" data-delta="-1" style="width:32px;height:32px;border-radius:50%;border:none;background:#eee;cursor:pointer;">-</button>
                <div style="min-width:28px;text-align:center;font-weight:700;">${item.quantity}</div>
                <button data-action="changeQty" data-id="${item.id}" data-delta="1" style="width:32px;height:32px;border-radius:50%;border:none;background:#4CAF50;color:white;cursor:pointer;">+</button>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding-top:12px;border-top:2px solid #e9f5ee;">
          ...
          <div style="display:flex;gap:10px;">
            ${cart.length > 0 ? `<button data-action="clearCart" style="padding:10px 12px;border-radius:10px;background:#f44336;color:white;border:none;cursor:pointer;">Очистить</button>` : ''}
            <button data-action="checkout" style="padding:10px 14px;border-radius:10px;background:linear-gradient(135deg,#667eea,#764ba2);color:white;border:none;cursor:pointer;" ${cart.length===0 ? 'disabled' : ''}>${cart.length===0 ? 'Добавьте товары' : 'Оформить'}</button>
          </div>
        </div>
      </div>
    </div>
...
async function updateQuantity(productId, delta){
  ...
  if (!item) return;
  const newQty = (item.quantity || 0) + Number(delta);
  if (newQty <= 0){
...
async function addToCart(productId){
  const p = teaCatalog.find(t => t.id === Number(productId));
...
async function checkout(){
  ...
  try {
    ...
    const order = {
      id: Date.now(),
      user_id: userId,
      user_name: userData ? (userData.first_name || 'Гость') : 'Гость',
      user_username: userData ? (userData.username || '') : '',
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div style="display:flex;gap:10px;margin-top:12px;">
          <button data-action="copyOrderText" style="flex:1;padding:10px;border-radius:8px;background:#4CAF50;color:white;border:none;cursor:pointer;">Копировать</button>
          <button data-action="openChat" style="flex:1;padding:10px;border-radius:8px;background:#2196F3;color:white;border:none;cursor:pointer;">Открыть чат</button>
        </div>
      </div>
    </div>
  `;
          ...
          ${orders.slice().reverse().map((o, idx) => `
            <div style="background:#f8f9fa;padding:12px;border-radius:10px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;">
              ...
              <div style="text-align:right;display:flex;flex-direction:column;gap:8px;">
                ...
                <div style="display:flex;gap:8px;">
                  <button data-action="showOrderDetails" data-id="${o.id}" style="padding:6px 8px;border-radius:8px;border:none;background:#fff;cursor:pointer;">Открыть</button>
                  <button data-action="reorder" data-id="${o.id}" style="padding:6px 8px;border-radius:8px;border:none;background:#4CAF50;color:white;cursor:pointer;">Повторить</button>
                </div>
              </div>
            </div>
          `).join('')}
  ...
  modal.innerHTML = `
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button class="modal-close" data-action="backToOrders">← Назад</button>
      </div>
      <div class="modal-body">
        ...
        <div style="display:flex;gap:8px;">
          <button data-action="copyOrderDetails" data-id="${order.id}" style="flex:1;padding:10px;border-radius:8px;background:#4CAF50;color:white;border:none;cursor:pointer;">Копировать</button>
          <button data-action="reorder" data-id="${order.id}" style="flex:1;padding:10px;border-radius:8px;background:#2196F3;color:white;border:none;cursor:pointer;">Повторить заказ</button>
        </div>
      </div>
    </div>
  `;
...
async function copyOrderDetails(orderId){
  ...
  lines.push(`Покупатель: ${order.user_name} ${userData && userData.username ? `(${userData.username})` : ''}`);
...
async function reorder(orderId){
  ...
  lines.push(`Покупатель: ${userData ? (userData.first_name || 'Гость') : 'Гость'} ${userData && userData.username ? `(${userData.username})` : ''}`);
  lines.push(`Сумма: ${order.total}₽`);
  lines.push('Товары:');
  order.cart.forEach(it => lines.push(` - ${it.name} × ${it.quantity} (${it.price}₽)`));
  lines.push('');
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <button data-action="openChannel" style="width:100%;padding:12px;margin-top:14px;background:linear-gradient(135deg,#4CAF50,#2E7D32);color:white;border:none;border-radius:10px;cursor:pointer;"><i class="fab fa-telegram"></i> Наш телеграм-канал</button>
      </div>
    </div>
  `;
  modal.style.display = 'flex';
...
window.addToCart = addToCart; // still available if needed
...
/* =========== CENTRALIZED CLICK HANDLER (delegation) =========== */
document.addEventListener('click', (e) => {
  ...
  if (btn){
    ...
    switch(act){
      case 'home': showMainInterface(); break;
      case 'catalog': showFullCatalog(); break;
      case 'showProduct': showProduct(id); break;
      case 'addToCart': addToCart(id); break;
      case 'cart': showCartModal(); break;
      case 'orders': showOrders(); break;
      case 'profile': showProfile(); break;
      case 'checkout': checkout(); break;
      case 'closeModal': closeAllModals(); break;
      case 'changeQty': updateQuantity(id, delta); break;
      case 'clearCart': clearCart(); break;
      case 'copyOrderText': copyOrderText(); break;
      case 'openChat': openChat(); break;
      case 'openChannel': openChannel(); break;
      case 'showOrderDetails': showOrderDetails(id); break;
      case 'reorder': reorder(id); break;
      case 'copyOrderDetails': copyOrderDetails(id); break;
      case 'backToOrders': showOrders(); break;
      default: break;
    }
    return;
  }

  // если клик по пустой области вне модального контента
...
/* =========== small loader used in checkout =========== */
function showLoader(message = 'Загрузка...'){ const loader = document.getElementById('loader'); if (!loader) return; const status = document.getElementById('loader-status'); if (status) status.textContent = message; loader.style.display = 'flex'; requestAnimationFrame(()=> loader.style.opacity = '1'); }
