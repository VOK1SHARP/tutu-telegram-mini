...
async function initApp() {
    try {
        ...
        UI.Loader.show('Загрузка приложения...');
        ...
        Utils.setIsTelegramUser(!!userData.id);
        ...
        console.log('[App] Пользователь:', {
            ...
            id: userId,
            isTelegram: Utils.getIsTelegramUser()
        });
        
        // Инициализация модулей
        Catalog.init();
        
        // Загрузка популярности и корзины параллельно
        const [popularity, cart] = await Promise.all([
            Storage.loadPopularity(),
            Cart.load()
        ]);
        
        // Обновление популярности
        await Storage.savePopularity(popularity);
        
        console.log('[App] Загружено:', {
            cartItems: cart.length,
            popularity: Object.keys(popularity).length
        });
        
        // Показ основного интерфейса
        showMainInterface();
        
        // Синхронизация данных через 1.5 секунды
        setTimeout(checkAndSyncData, 1500);
        ...
        setTimeout(() => {
            ...
            if (cart.length > 0) {
                UI.Toast.show(`Добро пожаловать! В корзине ${cart.length} товаров`);
            } else {
                UI.Toast.show(`Добро пожаловать, ${userData.first_name}!`);
            }
        ...
        UI.Loader.hide();
        UI.Toast.show('Ошибка загрузки приложения', { type: 'error', timeout: 5000 });
    }
}
...
>>>>>>> 534b713613ae22f2a376205cbeb1acfe5304de3c
    app.innerHTML = `
        <!-- Header -->
        <div class="header fade-in">
            <div class="header-content">
<<<<<<< HEAD
                <div class="logo" onclick="window.Catalog.showCatalog()" style="cursor: pointer;">
                ...
                <div class="user-avatar" onclick="window.Profile.showProfile()" 
                     title="${Utils.escapeHtml(fullName)}${username ? ` (${Utils.escapeHtml(username)})` : ''}">
                    ${hasPhoto ? 
...
<<<<<<< HEAD
            <h2>🍵 Добро пожаловать, ${Utils.escapeHtml(firstName)}!</h2>
            <p>${isTelegramUser ? 'Рады видеть вас снова!' : 'Аутентичный китайский чай с доставкой'}</p>
            <a href="#" class="banner-button" onclick="window.Catalog.showCatalog(); return false;">
                Смотреть каталог
            </a>
=======
...
<<<<<<< HEAD
            <div class="nav-item" onclick="window.Catalog.showCatalog()">
                <div class="nav-icon icon-tea"><i class="fas fa-mug-hot"></i></div>
                <h3>Каталог</h3>
                <p>${Catalog.getAllProducts().length}+ сортов чая</p>
            </div>
            <div class="nav-item" onclick="window.Orders.showOrdersHistory()">
                <div class="nav-icon icon-orders"><i class="fas fa-box"></i></div>
                <h3>Заказы</h3>
                <p>История покупок</p>
            </div>
            <div class="nav-item" onclick="window.Cart.showCartModal()">
                <div class="nav-icon icon-cart"><i class="fas fa-shopping-cart"></i></div>
                <h3>Корзина</h3>
                <p>Товары: <span class="cart-count">0</span></p>
            </div>
            <div class="nav-item" onclick="window.Profile.showProfile()">
...
async function loadPopularProducts() {
    try {
        const popularity = await Storage.loadPopularity();
        const popularProducts = Catalog.getPopularProducts(popularity, 4);
        ...
        popularProducts.forEach(product => {
            const productCard = UI.createProductCard(product, (productId) => {
                Catalog.showProductDetail(productId);
            });
...
async function checkout() {
    try {
        if (Cart.isEmpty()) {
            UI.Toast.show('Добавьте товары в корзину', { type: 'error' });
            return;
        }
        ...
        // Обновление популярности
        const popularity = await Storage.loadPopularity();
        const newPopularity = Storage.updatePopularityFromOrder(order, popularity);
        await Storage.savePopularity(newPopularity);
        
        // Копирование текста заказа
        const copied = await Orders.copyOrderText(order);
        ...
        if (copied) {
            UI.Toast.show('Заказ создан! Текст скопирован в буфер.', { timeout: 5000 });
        } else {
            UI.Toast.show('Заказ создан! Откройте чат менеджера.', { timeout: 5000 });
        }
        ...
        UI.Toast.show('Ошибка оформления заказа', { type: 'error', timeout: 5000 });
    } finally {
        UI.Loader.hide();
    }
}
...
async function checkAndSyncData() {
    try {
        ...
        if (userData.id && tg && tg.CloudStorage) {
            // Проверяем облачную корзину
            const cloudCart = await new Promise(resolve => {
                tg.CloudStorage.getItem('cart', (err, val) => {
                    resolve(!err && val ? val : null);
                });
            });
            
            if (cloudCart) {
                const parsedCart = JSON.parse(cloudCart);
                const localKey = Utils.getAppKeys().CART_KEY(userId);
                const localCart = localStorage.getItem(localKey);
                
                // Если в облаке более свежие данные
                if (!localCart || (Array.isArray(parsedCart) && parsedCart.length > JSON.parse(localCart || '[]').length)) {
                    await Storage.saveCart(parsedCart);
                    Cart.updateUI();
                    UI.Toast.show('Корзина синхронизирована из облака');
                    console.log('[App] Корзина синхронизирована из облака');
                }
            }
        }
    } catch (error) {
        console.warn('[App] Ошибка синхронизации:', error);
    }
}
